function runTest(){
  var JsDiff = FBTest.FirebugWindow.JsDiff, FBTrace = FBTest.FirebugWindow.FBTrace;

  // Whitespace diff (Single whitespace character)
  diffResult = JsDiff.diffWords("New Value", "New  ValueMoreData");
  FBTest.compare(
      "New  <ins>ValueMoreData</ins><del>Value</del>",
      JsDiff.convertChangesToXML(diffResult),
      "Single whitespace diffResult Value");

  // Whitespace diff (Multiple whitespace characters)
  diffResult = JsDiff.diffWords("New Value  ", "New  ValueMoreData ");
  FBTest.compare(
      "New  <ins>ValueMoreData</ins><del>Value</del> ",
      JsDiff.convertChangesToXML(diffResult),
  ...
  diffResult = JsDiff.diffWords("New :Value:Test", "New  ValueMoreData ");
  FBTest.compare(
      "New  <ins>ValueMoreData </ins><del>:Value:Test</del>",
      JsDiff.convertChangesToXML(diffResult),
      "Nonmatching word boundary diffResult Value");
  diffResult = JsDiff.diffWords("New Value:Test", "New  Value:MoreData ");
  FBTest.compare(
  ...
  diffResult = JsDiff.diffWords("New Value", "New  Value:MoreData ");
  FBTest.compare(
  ...
  diffResult = JsDiff.diffWords("New Value", "New Value");
  FBTest.compare(
      "New Value",
      JsDiff.convertChangesToXML(diffResult),
  ...
  // Empty diffs
  diffResult = JsDiff.diffWords("New Value", "");
  FBTest.compare(1, diffResult.length, "Empty diff result length");
  FBTest.compare(
      "<del>New Value</del>",
      JsDiff.convertChangesToXML(diffResult),
  ...
  diffResult = JsDiff.diffWords("New Value New Value", "Value Value New New");
  FBTest.compare(
  ...
  diffResult = JsDiff.diffLines(
      "line\nvalue \nline",
      "line\nvalue\nline");
  FBTrace.sysout("diffResult", diffResult);
  FBTrace.sysout("diffResult", JsDiff.convertChangesToXML(diffResult));
  FBTest.compare(
      "line\n<ins>value\n</ins><del>value \n</del>line",
      JsDiff.convertChangesToXML(diffResult),
  ...
  FBTrace.sysout("len: " + len + " count: " + count + " removed ( " + logData.join(", ") + " )");

  diffResult = JsDiff.diffWords(largeTest, largeNewValue);
  FBTrace.sysout("diffResult length: " + diffResult.length);
  var removeCount = 0;
  var removeChanges = [], addChanges = [], testChanges = [];
  for (var i = 0; i < diffResult.length; i++) {
    if (diffResult[i].removed) {
      FBTrace.sysout("remove Change " + i, diffResult[i]);
      removeChanges.push(diffResult[i].value);
    } else if (diffResult[i].added) {
      FBTrace.sysout("add Change " + i, diffResult[i]);
      addChanges.push(diffResult[i].value);
    } else {
      FBTrace.sysout("no Change " + i, diffResult[i]);
      removeChanges.push(diffResult[i].value);
      addChanges.push(diffResult[i].value);
    }
  }

  FBTrace.sysout("diffResult remove length: " + removeCount);
  FBTest.compare(largeTest.replace(/s+/g, ""), removeChanges.join("").replace(/s+/g, ""), "New Diff results match");
  FBTest.compare(largeNewValue.replace(/s+/g, ""), addChanges.join("").replace(/s+/g, ""), "Old Diff results match");
  ...
  diffResult = JsDiff.createPatch("testFileName", oldFile, newFile, "Old Header", "New Header");
  FBTest.compare(
      expectedResult,
      diffResult,
